...
export class JoinigFormComponent implements OnInit {
  ...
  isValidList = [false, false, false, false, false];
  ...
  next() {
    ...
    if (this.subscription.gender == 2 && this.caurselPos == 2) {
      ...
      if (this.caurselPos != 2)
        this.isFormValid = this.isValidList[this.caurselPos];
    }
  }
  ...
  output(event: any) {
    ...
    if (this.caurselPos == 0) {
      ...
      this.subscription.birthday_Gregorian = isDateHijri
        ? this.dateSrv.gergorianToString(birthDate, '-')
        : this.dateSrv.replaceNumber(imoment(birthDate).format('YYYY-MM-DD'));
      this.subscription.birthday_Hiri = isDateHijri
        ? this.dateSrv.replaceNumber(imoment(birthDate).format('YYYY-MM-DD'))
        : this.dateSrv.hijriToString(birthDate, '-');
      ...
      this.isValidList[0] = this.isFormValid;
      this.logger.log('subscription 1:', this.subscription);
    } else if (this.caurselPos == 1) {
      this.subscription.nature_of_daily_stress =
        event.data.workNature == 'office'
          ? 'مكتبي'
          : event.data.workNature == 'field'
          ? 'ميداني'
          : event.data.workNature;
      this.subscription.health_problems =
        event.data.healthProblems == 'no'
          ? 'لايوجد'
          : event.data.healthProblems;
      this.subscription.supplement_use =
        event.data.supplements == 'no' ? 'لايوجد' : event.data.supplements;
      ...
      this.subscription.wakeup_time = event.data.weakupTime
        .replace(' AM', '')
        .replace(' PM', '');
      this.subscription.current_diet_program =
        event.data.ditePlan == 'no' ? 'لايوجد' : event.data.ditePlan;
      this.subscription.number_of_meals =
        event.data.numberMeals == 'no' ? 'لايوجد' : event.data.numberMeals;
      this.subscription.Allergens_food =
        event.data.allergen == 'no' ? 'لايوجد' : event.data.allergen;
      this.subscription.unfavorable_food =
        event.data.unlike == 'no' ? 'لايوجد' : event.data.unlike;
      this.subscription.surgeries_history =
        event.data.operations == 'no' ? 'لايوجد' : event.data.operations;
      this.logger.log('subscription 2:', this.subscription);
      this.isValidList[1] = this.isFormValid;
      ...
      this.isValidList[2] = this.isFormValid;
      ...
      this.isValidList[3] = this.isFormValid;
    } else if (this.caurselPos == 4) {
      this.isValidList[4] = this.isFormValid;
    ...
    this.logger.log('isValidList:', this.isValidList);
  }
